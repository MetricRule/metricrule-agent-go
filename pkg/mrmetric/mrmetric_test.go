...
func TestGetAggregatorSpecHistogramBin(t *testing.T) {
	configTextProto := `
		output_metrics {
			value {
				value {
	...
	wantLen := 1
	if gotLen != wantLen {
		t.Errorf("Expected %v aggregator specs, got %v", wantLen, gotLen)
	}

	for instr, agg := range specs {
		gotInstrKind := instr.InstrumentKind
		wantInstrKind := metric.ValueRecorderInstrumentKind
		if gotInstrKind != wantInstrKind {
			t.Errorf("Expected instrument kind %v, got %v", wantInstrKind, gotInstrKind)
		}

		gotValueKind := instr.MetricValueKind
		wantValueKind := reflect.Float64
		if gotValueKind != wantValueKind {
			t.Errorf("Expected value kind %v, got %v", wantValueKind, gotValueKind)
		}

		gotHistogramBins := agg.HistogramBins
		wantHistogramBins := []float64{0.0, 0.25, 0.5, 0.75, 1.0}
		if !reflect.DeepEqual(gotHistogramBins, wantHistogramBins) {
			t.Errorf("Expected bins %v, got %v", wantHistogramBins, gotHistogramBins)
		}
	}
}

func TestGetAggregatorSpecValueNoHistogramBin(t *testing.T) {
	configTextProto := `
		input_metrics {
			name: "simple"
			simple_counter: {}
	...
	for instr, agg := range specs {
		gotInstrKind := instr.InstrumentKind
		wantInstrKind := metric.CounterInstrumentKind
		if gotInstrKind != wantInstrKind {
			t.Errorf("Expected instrument kind %v, got %v", wantInstrKind, gotInstrKind)
		}

		gotValueKind := instr.MetricValueKind
		wantValueKind := reflect.Int64
		if gotValueKind != wantValueKind {
			t.Errorf("Expected value kind %v, got %v", wantValueKind, gotValueKind)
		}

		gotHistogramBins := agg.HistogramBins
		if gotHistogramBins != nil {
			t.Errorf("Expected bins nil, got %v", gotHistogramBins)
		}
	}
}

func TestGetAggregatorSpecCounter(t *testing.T) {
	configTextProto := `
		output_metrics {
			value {
				value {
